...
class BenchmarkRunner:
    ...
    def run(self) -> dict:
        ...
        _pt = float(getattr(cfg.backtest, "proba_threshold", 0.0) or 0.0)
        if _pt > 0.0:
            console.print(
                f"  [yellow]proba_threshold={_pt:.3f}[/yellow] active — framework "
                f"metrics computed on filtered signals (bars with confidence < "
                f"{_pt:.3f} treated as flat). Feather stores raw labels + "
                f"per-bar confidence so the threshold can be retuned later."
            )
    ...
    def _fit_eval_loop(
        ...
        # Confidence-based signal filter (same semantic as the freqtrade
        # strategy generated by `regime-run backtest`): bars below the
        # threshold get their predicted class collapsed to the flat
        # neutral (0 for numeric arrays, "" for string arrays) BEFORE we
        # evaluate gain / sharpe / count_trades. 0.0 disables.
        proba_threshold = float(getattr(self.cfg.backtest, "proba_threshold", 0.0) or 0.0)

        def _apply_proba_filter(pred_arr: np.ndarray, confidence: np.ndarray) -> np.ndarray:
            if proba_threshold <= 0.0 or confidence is None:
                return pred_arr
            if len(confidence) != len(pred_arr):
                return pred_arr  # shape mismatch -> safe fallback
            out = pred_arr.copy()
            weak = np.asarray(confidence) < proba_threshold
            if pred_arr.dtype.kind in ("i", "f", "b", "u"):
                out[weak] = 0
            else:  # object / string arrays
                out[weak] = ""
            return out

        def _evaluate(predictor, pred_arr, t0, confidence=None):
            # Filter is applied ONLY to the array fed into evaluate() —
            # per_predictor_predictions keeps the RAW labels so the feather
            # written downstream can be re-filtered at a different threshold
            # without rerunning the framework.
            pred_arr_for_metrics = (
                _apply_proba_filter(pred_arr, confidence)
            ...
            results.append(res)
            per_predictor_predictions[predictor.name] = pred_arr  # raw, unfiltered
            dk_str = (
        ...
        for predictor in base_predictors:
            ...
            with console.status(f"[bold green]{predictor.name}[/bold green]"):
                ...
                try:
                    ...
                    # Compute proba FIRST so we can filter pred_arr by
                    # confidence BEFORE _evaluate computes framework metrics.
                    # Bases that lack predict_proba get conf=1.0 -> no filter.
                    try:
                    ...
                    _evaluate(
                        predictor, pred_arr, t0,
                        confidence=per_predictor_confidences[predictor.name],
                    )
                except Exception as e:
        ...
        for predictor in ensemble_predictors:
            with console.status(f"[bold green]{predictor.name}[/bold green]"):
                ...
                try:
                    ...
                    pred_arr = np.asarray(predictor.predict(X_te, d_te, df_te))
                    # Ensembles can expose their own predict_proba (avg of
                    # base probas weighted by confidence/kappa). Capture it
                    # FIRST to filter pred_arr before _evaluate, matching
                    # the freqtrade strategy semantic.
                    try:
